...
def _run_cpp(files, tmp_dir, stdin_text, started) -> dict:
    ...
    if list(files.keys()) == ["main"]:
        ...
        if not cpp_files:
            return {"ok": False, "stage": "compile", "language": "cpp",
    ...
    if rc != 0 or timed_out:
        return _result("cpp", "compile", rc, out, err, timed_out, started)

    rc, out, err, timed_out = _run_proc(
        [str(Path(tmp_dir) / exe_name)], tmp_dir, stdin_text, RUN_TIMEOUT_S
    )
...
def _run_java(files, tmp_dir, stdin_text, started) -> dict:
    ...
    if list(files.keys()) == ["main"]:
        ...
        if not java_files:
            return {"ok": False, "stage": "compile", "language": "java",
                     "exit_code": None, "stdout": "", "timed_out": False,
    ...
    rc, out, err, timed_out = _run_proc(compile_cmd, tmp_dir, None, COMPILE_TIMEOUT_S)
    if rc != 0 or timed_out:
        return _result("java", "compile", rc, out, err, timed_out, started)
